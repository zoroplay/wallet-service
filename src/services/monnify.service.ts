...
export class MonnifyService {
    ...
    try {
      ...
      if (authRes.requestSuccessful) {
        const data = await get(
          `${paymentSettings.base_url}/api/v1/merchant/transactions/query?paymentReference=${param.transactionRef}`,
          {
    ...
    try {
      ...
      if (authRes.requestSuccessful) {
        ...
        const resp = await post(
          `${paymentSettings.base_url}/api/v2/disbursements/single`,
          {
        ...
        return {
          success: resp.requestSuccessful,
          data: resp.data,
      ...
      return {
        success: false,
        message: 'Monnify Error! unable to disburse funds',
        status: HttpStatus.BAD_REQUEST,
      };
    }
  }
  ...
  private async authenticate(paymentSettings: PaymentMethod) {
    const key = btoa(
      `${paymentSettings.public_key}:${paymentSettings.secret_key}`,
    );

    return await post(
      `${paymentSettings.base_url}/api/v1/auth/login`,
      {},
      {
  ...
  async generatePaymentLink(data, client_id) {
    try {
      ...
      if (authRes.requestSuccessful) {
        const resp = await post(
          `${paymentSettings.base_url}/api/v1/merchant/transactions/init-transaction`,
          {
            amount: data.amount,
      ...
      switch (data.event) {
        ...
        case 'SUCCESSFUL_DISBURSEMENT':
          ...
          if (withdrawalSuccess && withdrawalSuccess.status === 0) {
            ...
            console.log(
              'transfer.success: withdrawal not found',
              data.reference,
            );
          }
          break;
        case 'FAILED_DISBURSEMENT':
          ...
          if (withdrawalFailed) {
            ...
            console.log(
              'transfer.failed: withdrawal not found',
              data.reference,
            );
          }
          break;
