...
export class PaymentService {
  ...
  async inititateDeposit(
    ...
    console.log('CHECK');
    await this.identityService.createLog({
      logs: [
        {
          id: 0,
          userId: param.userId,
          ...
          endpoint: clientMetadata.endpoint,
          method: 'POST',
          statusCode: 200,
          payload: JSON.stringify(param), // Ensure it's a string
          response: null, // Response will be logged after processing
          additionalInfo: {
          ...
          const res = await this.pawapayService.generatePaymentLink(
            {
              ...
              returnUrl:
                user.callbackUrl + '/payment-verification/pawapay' ||
                `https://api.staging.sportsbookengine.com/api/v2/webhook/4/pawapay/callback`,
        ...
        logs: [
          {
            ...
            userId: param.userId,
            clientId: param.clientId,
            action: 'INITIATE_DEPOSIT_FAILED',
            endpoint: clientMetadata.endpoint,
            method: 'POST',
            statusCode: 500,
            payload: JSON.stringify(param), // Ensure it's a string
  ...
  async createVirtualAccount(param: WayaBankRequest, clientMetadata: any) {
    try {
      const wallet = await this.walletRepository
        .createQueryBuilder()
        .where('client_id = :clientId', { clientId: param.clientId })
      ...
      await this.identityService.createLog({
        logs: [
          {
            id: 0, // Auto-generated by Prisma
            userId: param.userId,
            clientId: param.clientId,
            action: 'CREATE_VIRTUAL_ACCOUNT',
            endpoint: clientMetadata.endpoint, // Dynamically set
            method: 'POST',
            statusCode: 200,
            payload: JSON.stringify(param), // Ensure it's a string
  ...
  async createRequest(param, clientMetadata: any) {
    try {
      const wallet = await this.walletRepository
        .createQueryBuilder()
        .where('client_id = :clientId', { clientId: param.clientId })
