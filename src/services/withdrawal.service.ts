...
export class WithdrawalService {
  ...
  async requestWithdrawal(data: WithdrawRequest, clientMetadata: any): Promise<WithdrawResponse> {
    // console.log(data);
    try {
        ...
        logs: [
          {
            ...
            method: 'POST',
            statusCode: 200,
            payload: JSON.stringify(data), // Ensure it's a string
            response: JSON.stringify({ success: true, message: 'Withdrawal request received' }),
            additionalInfo: {
        ...
        logs: [
          {
            ...
            userId: data.userId,
            clientId: data.clientId,
            action: 'WITHDRAWAL_REQUEST_FAILED', // Updated action for failed withdrawal
            endpoint: clientMetadata.endpoint,
            method: 'POST',
            statusCode: 400, // or any other error status
            payload: JSON.stringify(data),
            response: JSON.stringify({ success: false, message: 'Insufficient funds' }), // Customize based on error
            additionalInfo: {
              browser: clientMetadata.browser,
              os: clientMetadata.os,
              platform: clientMetadata.platform,
