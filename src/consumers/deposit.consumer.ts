...
export class DepositConsumer {
    ...
    async processShopWithdrawal(job: Job<unknown>) {
        try {
             console.log(
                `Processing deposit job ${job.id} of type ${job.name}...`,
            );

            const data: any = job.data;
            // find initiator wallet
            let fromWallet = await this.walletRepository.findOne({where: {user_id: data.fromUserId, client_id: data.clientId}});
            // find receiver wallet
            let toWallet = await this.walletRepository.findOne({where: {user_id: data.toUserId, client_id: data.clientId}});
                
            //update request status
            const senderBalance = fromWallet.available_balance - data.amount;
            // credit receiver balance
            const receiverBalance = Number(toWallet.available_balance) + Number(data.amount);
            ...
            await this.transactionRepository.update({transaction_no: data.transactionCode}, {status: 1})
