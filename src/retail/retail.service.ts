...
export class RetailService {
    ...
    async listLast10Transactions({userId}) {
        try {
            ...
            return {success: true, message: 'Transaction retrieved', status: HttpStatus.OK, data: JSON.stringify(result)}
        } catch (e) {
    ...
    async balanceOverview({userId}) {
        try {
            ...
            return {success: true, message: 'Transaction retrieved', status: HttpStatus.OK, data: JSON.stringify(data)}
        } catch (e) {
            return {success: false, status: HttpStatus.INTERNAL_SERVER_ERROR, message: `Something went wrong: ${e.message}`};
        }
    }
